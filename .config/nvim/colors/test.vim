hi clear Normal
set bg&
hi clear

hi Constant guifg=#aabbee gui=none
hi @string.c guifg=#aabbee gui=none
hi @number.c guifg=#2288aa gui=none

hi Statement guifg=#cc6600 gui=none
hi @operator.c guifg=#7733aa gui=none
hi @operator.cpp guifg=#7733aa gui=none
hi @keyword.directive.define.c guifg=#bb99aa gui=none
hi @keyword.c guifg=#aa3377 gui=NONE
hi @keyword.cpp guifg=#aa3377 gui=NONE
hi @keyword.import.c guifg=#aaaaaa gui=none
hi @keyword.import.cpp guifg=#aaaaaa gui=none

hi Type guifg=#aa3377 gui=NONE
hi @type.c guifg=#aa3377 gui=NONE
hi @type.builtin.c guifg=#aa3377 gui=NONE

hi PreProc guifg=#bb33ee gui=NONE

hi Identifier guifg=#aaaaaa gui=NONE
hi @variable.c guifg=#aa8888 gui=NONE
hi @variable.parameter.c guifg=#aa8888 gui=NONE
hi @lsp.type.parameter.c guifg=#aa8888 gui=NONE
hi @lsp.type.function.c guifg=#aaaaaa gui=none

hi @variable.cpp guifg=#aa8888 gui=NONE
hi @variable.parameter.cpp guifg=#aa8888 gui=NONE
hi @lsp.type.parameter.cpp guifg=#aa8888 gui=NONE
hi @lsp.type.function.cpp guifg=#aaaaaa gui=none

hi Special guifg=#aaaaaa gui=NONE

hi LineNr guifg=#aaaaaa
hi CursorLineNr guifg=#cc5555

hi Pmenu guifg=#aaaaaa guibg=#161616
hi PmenuSel guifg=#000000 gui=BOLD

hi NonText guifg=#aaaaaa

hi MatchParen guifg=#9988cc guibg=NONE
